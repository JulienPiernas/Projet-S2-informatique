...
class Acheteur:
    def __init__(self, salaire, prix_limite, besoins):
        ...
        self.humeur = "Content"
        self.acheté_en_dernier = 0  # Ajout de l'attribut manquant
    ...
    def achete(self, prix, vendeur):
        ...
        self.acheté_en_dernier = nb_acheté  # Stockage du nombre d'articles achetés
    ...
    def changer_besoins(self):
        self.besoins += randint(1, 5)
        var = self.humeur
        if 'Pas content' not in var:
            self.besoins -= self.acheté_en_dernier

    def change_prix_limite(self):
        if "Moyen" in self.humeur:
            self.pl *= 1.05
        elif "Pas content" in self.humeur:
            self.pl *= 1.1
        else:
            self.pl *= 0.9
        if self.pl > self.brs:
...
def enregistrer_resultats(vendeur, liste_acheteurs, iteration):
    # Au début de la simulation, on efface le contenu du fichier
    if len(vendeur.recettes) == 0:  # Si c'est la première itération
