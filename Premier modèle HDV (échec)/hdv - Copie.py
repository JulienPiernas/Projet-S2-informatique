class Acheteur:
    def __init__(self, prix_max, brs, produit_voulu):
        self.prix = prix_max
        self.brs = brs
...
class Vendeur:
    def __init__(self, stock, brs, prix_min):
        self.stock = stock
        self.prix = prix_min
        self.brs = brs
        self.brs_apres_tick = brs
...
class HDV:
    ...
    def recherche_ordre_vente(self, ordre_achat):
        for ordre in self.ordres_vente:
            if ordre.produit == ordre_achat.produit and ordre.prix <= ordre_achat.prix:
